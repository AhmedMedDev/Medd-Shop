...
// Session
if (isset($request->post['token']) && isset($request->get['route']) && substr($request->get['route'], 0, 4) == 'api/') {
	$db->query("DELETE FROM `" . DB_PREFIX . "api_session` WHERE TIMESTAMPADD(HOUR, 24*7, date_modified) < NOW()");

	$query = $db->query("SELECT DISTINCT * FROM `" . DB_PREFIX . "api` `a` LEFT JOIN `" . DB_PREFIX . "api_session` `as` ON (a.api_id = as.api_id) WHERE a.status = '1' AND as.token = '" . $db->escape($request->post['token']) . "'");
...
if (isset($request->post['language']) && array_key_exists($request->post['language'], $languages)) {
...
//$registry->set('cart', new CartHelper($registry));
