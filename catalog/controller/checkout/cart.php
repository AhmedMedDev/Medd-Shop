<?php
	...
	class ControllerCheckoutCart extends Controller {
		...
		public function share() {
			...
			if (!$this->customer->isLogged()) {
				$this->session->data['redirect'] = $this->url->link('checkout/cart/share', '&cart_id=' . $cart_id, 'SSL');
				$this->response->redirect($this->url->link('account/login', '', 'SSL'));
			}
			...
			if ($this->cart->checkCartUser($cart_id, $customer_id)) {
				...
				$this->session->data['error'] = $this->language->get('error_user_already_member');
				$this->response->redirect($this->url->link('checkout/cart', '', 'SSL'));
			}
			...
			if ($cart_sharing_approved == 1) {
				...
				$this->session->data['success'] = $this->language->get('success_user_added_in_cart');
				$this->response->redirect($this->url->link('checkout/cart', '', 'SSL'));
				} else if ($cart_sharing_approved == 2) {
				$this->session->data['success'] = $this->language->get('success_cart_sharing_declined');
				$this->response->redirect($this->url->link('checkout/cart', '', 'SSL'));
				} else {
		...
		public function index() {
			...
			if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
				...
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/error/cart_empty.tpl')) {
					$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/error/cart_empty.tpl', $data));
					} else {
			...
			if(isset($cart_data['mc_current_stage']) && ($cart_data['mc_current_stage']==1 || $cart_data['mc_current_stage']==2 )&& $cart_data['mc_checkout_by_user']!=$current_logged_customer_id){
                ...
                if ($product_info) {
                    ...
                    if($this->request->post['quantity'] < $product_info['minimum']) {
                        ...
                        $json['error']['stock'] = sprintf($this->language->get('error_minimum_product_count'), $product_info['minimum']); 
                        $json['status'] = 0; 
                    }
                    
                    /* if($this->request->post['quantity'] == 1 && $product_info['minimum'] > 1) {
                        $this->request->post['quantity'] = $product_info['minimum'];
                    } */
                    
                    $total_asked_quantity = (int)$this->request->post['quantity'] + $q;                  
                    //echo 'total = '.$total_asked_quantity;die;
                    if($product_info['quantity'] == 0 ){
						$proceed_to_cart = false;
						$json['error']['stock'] = $this->language->get('error_stock_app'); 
						$json['status'] = 0;
					} 
                    else if((int)$product_info['quantity'] < $total_asked_quantity){
                        $proceed_to_cart = false;
						$json['error']['stock'] =sprintf($this->language->get('error_stock_available'), $product_info['quantity']);
						$json['status'] = 0;
                    }
		...
		public function edit() {
			...
			if (!empty($this->request->post['quantity'])) {
				...
				if(isset($cart_data['mc_current_stage']) && ($cart_data['mc_current_stage']==1 || $cart_data['mc_current_stage']==2 )&& $cart_data['mc_checkout_by_user']!=$current_logged_customer_id){
					...
					foreach ($this->request->post['quantity'] as $product_id => $value) {    
                          
                            $product_info = $this->model_catalog_product->getProduct($product_id);
                            
                            $q = 0;    
                            $total_asked_quantity = 0;
                            if(array_key_exists($product_id,$qdata)){                        
                                $q = $qdata[$product_id];
                            }                    
                             
                            
                            foreach ($value as $cartProductId => $val) {
                                
                                if($val < $product_info['minimum']) {
                                    $proceed_to_cart = false;
                                    $json['error'] = sprintf($this->language->get('error_minimum_product_count'), $product_info['minimum']); 
                                    $json['status'] = 0;
                                }
                                ...
                                else if((int)$product_info['quantity'] < $val){
                                    echo 'else if = '.$val;
                                    $proceed_to_cart = false;
                                    ...
                                    $this->cart->update($cartProductId, $val);
                                }
