...
class ControllerAccountReturn extends Controller {
	...
	public function index() {
		...
		$data['add_return'] = $this->url->link('account/return/add', '', 'SSL');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
           ...
           /*  echo '<pre>';
			...
			$order_status_info = $this->model_account_order->getOrderStatus();
			$allow_return_status = $this->config->get('config_allow_return_status');

			if($this->model_account_return->checkIfRequestAlreadyPlaced((int)$this->request->post['order_id'], $this->request->post['model'])){

				$this->session->data['error'] = $this->language->get('error_return_already_placed');
				$this->response->redirect($this->url->link('account/return/add', '', 'SSL'));
			}

			if(in_array($order_info['order_status_id'], $allow_return_status)){
				...
				$allowed_status_names = array();
				foreach($allow_return_status as $status){
					$allowed_status_names[] = $order_status_info[$status];
				}

				$this->session->data['error'] = sprintf($this->language->get('error_return_status'), implode(' or ',$allowed_status_names));
				$this->response->redirect($this->url->link('account/return/add', '', 'SSL'));
			}
		...
		$data['account_sidebar'] = $this->load->controller('account/account_sidebar');
		$data['add_return'] = $this->url->link('account/return', '', 'SSL');
		$data['button_return'] = $this->language->get('button_return');
