...
class ControllerToolAffiliateProfileUpload extends Controller {
	public function index() {
		
		if (!$this->affiliate->isLogged()) {
			$json = array();
			$json['profile'] = false;
			$json['success'] = $this->url->link('affiliate/account', '', 'SSL');
			...
			if (!$json) {
				...
				$result = $this->model_tool_affiliate_profile_upload->addUpload('',$newfilename);
				$json['profile'] = false;
				if($result!=false){
					...
					if(is_array($result) && count($result)>0){
						if (is_file(DIR_PROFILE_UPLOAD . $result['profile_image'])) { 
							...
							$json['profile_image_1'] = $this->model_tool_image->resizeProfileImage($result['profile_image'], 101, 101);
							
							$json['profile_image_2'] = $this->model_tool_image->resizeProfileImage($result['profile_image'], 81, 81);
