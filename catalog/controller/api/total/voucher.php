<?php
class ControllerAPITotalVoucher extends Controller {
	...
	public function voucher() {
		...
		$this->load->model('api/total/voucher');
		...
		if (empty($this->request->post['voucher'])) {
			...
			$json['voucher'] = $this->request->post['voucher'];
			$this->model_api_total_voucher->applyVoucher($cart_id,$voucher);
	...
	public function deleteVoucher(){
		...
		if($cart_id){
			$this->model_api_total_voucher->deleteVoucher($cart_id);
		...
		if (in_array($order_info['order_status_id'], $this->config->get('config_complete_status'))) {
			...
			if ($voucher_query->num_rows) {
				...
				foreach ($voucher_query->rows as $voucher) {
					...
					$mailHelper->sendEmail($voucher['to_email'], sprintf($language->get('text_subject'), $voucher['from_name']), $html);
