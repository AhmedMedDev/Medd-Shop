<?php
class ControllerAPITotalShipping extends Controller {
	...
	public function quote() {
		...
		if (!$json) {
			...
			$serializeSessionData = serialize($this->session->data);
			
			$customerToken = $this->customer->getToken();
			
			$this->customer->addSession($customerToken,$serializeSessionData);
	...
	public function shipping() {
		$this->load->language('total/shipping');
		$this->load->model('api/total/shipping');
		$json = array();
		...
		$sessionData = (!empty($sessionData['session_data'])) ? unserialize($sessionData['session_data']) : $method_data;
		 
		$this->session->data['shipping_methods'] = (!empty($sessionData['shipping_methods'])) ? $sessionData['shipping_methods'] : $method_data;;
		...
		if (!$json) {
			...
			$this->model_api_total_shipping->applyShipping($cart_id,$this->request->post['shipping_method']);
	...
	public function deleteShipping(){
		...
		if($cart_id){
			$this->model_api_total_shipping->deleteShipping($cart_id);
