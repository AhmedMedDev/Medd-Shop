...
class ControllerAPICommonAnylist extends Controller {
	public function index($setting) {
		...
		if ($enabled) {
	        ...
	        $data = array();
	   		$data['heading_title'] = isset($setting['title'][$languageId])?$setting['title'][$languageId]:$setting['title'][1];
			...
			$template = isset($setting['template']) && ($setting['template']!='') ? $setting['template'] : 'anylist';
			
			$data['html_block_offers'] = '';
			if($template=='anylist_product_offers'){
				
				/*	Needs to update this id later on as per products offers module	*/
				$anylist_product_offers_html_module_id = ((int)$this->config->get('anylist_product_offers_html_module_id')!=0) ? (int)$this->config->get('anylist_product_offers_html_module_id') : 46;
				//echo $anylist_product_offers_html_module_id;
				$anylist_modules = $this->model_extension_module->getModule($anylist_product_offers_html_module_id);//anylist module for new product
	...
	public function listall() {
		$languageId = $this->config->get('config_language_id');
		$this->language->load('api/category');
		...
		if (!$filter or $ok) {
			...
			foreach ($results as $prod_number) {
				...
				if ($result) {
					...
					$price_without_currency = false;
						
					if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
						
						$price_without_currency = $this->tax->calculate($result['price'], $result['tax_class_id'], $this->config->get('config_tax'));//additional
					...
					$special_without_currency = false;
					
					if ((float)$result['special']) {
					
						$special_without_currency = $this->tax->calculate($result['special'], $result['tax_class_id'], $this->config->get('config_tax'));//additional for offers page
                  ...
                  $resultingProducts[] = array(
						...
						'price'   	 => $price,
						'special' 	 => $special,
						'price_without_currency' => ((isset($price_without_currency) && $price_without_currency!=false) ? $price_without_currency : ''),
						'special_without_currency' => ((isset($special_without_currency) && $special_without_currency!=false) ? $special_without_currency : ''),
                        'tax'        => $tax,
						'rating'     => $rating,
