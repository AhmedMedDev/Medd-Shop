<?php
class ControllerAPICommonCurrency extends Controller {
	...
	public function currency() {
		...
		if (isset($this->request->post['code'])) {
			...
			$this->model_api_account_customer->updateCurrency($this->request->post['code'],$customer_id);
			/* unset($this->session->data['shipping_method']);
			unset($this->session->data['shipping_methods']); */
			$json['success'] = 'currency updated';
			$json['status'] = 1;
		}else{
			$json['error'] = 'please select Currency code';
			$json['status'] = 0;
		}
