<?php
class ControllerAPIAccountReturn extends Controller {
	private $error = array();
	...
	public function getReasons() {
		...
		$data['reasons'] = $this->model_localisation_return_reason->getReturnReasons();
		$data['status'] = 1;
		$this->response->setOutput(json_encode($data));
	...
	public function add() {
		$this->load->language('api/account/return');
		$this->load->model('account/api');
		$this->load->model('api/account/return');
		/* 
		echo $header;
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$return_id = $this->model_api_account_return->addReturn($this->request->post,$customerId);
			...
			if ($customerId) {
				...
				$this->model_api_account_activity->addActivity('return_guest', $activity_data);
			}
			$data['status'] = 1;
