<?php
	class ControllerProductProduct extends Controller {
		...
		public function index() {
			...
			$this->document->addScript('catalog/view/theme/growcer/js/eagle.gallery.js');
			...
			if ($product_info) {
				...
				if (($this->config->get('config_tax') && ($this->config->get('config_customer_price') && $this->customer->isLogged())) || ($this->config->get('config_tax') && !$this->config->get('config_customer_price'))) {
		...
		public function write() {
			...
			if ($this->request->server['REQUEST_METHOD'] == 'POST') {
				...
				if (!isset($json['error'])) {
					...
					if ($this->config->get($this->config->get('config_captcha') . '_status') && in_array('review', (array)$this->config->get('config_captcha_page'))) { 
						$json['captcha'] = 'reload';
						$json['success'] = $this->language->get('text_success_reload');
						} else {
						$json['captcha'] = '';
					}
				}
