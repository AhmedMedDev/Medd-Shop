...
class ControllerProductCompare extends Controller {
	public function index() {
		...
		if($this->customer->isLogged()){
			$this->model_product_compare->removeProducts();
			$this->model_product_compare->addCompareProducts($this->session->data['compare']);
		...
		if (isset($this->request->get['remove'])) {
			...
			if ($key !== false) {
				if($this->customer->isLogged()){
					$this->removeProductFromList($this->session->data['compare'][$key]);
				}
	...
	public function getCompareProductList(){
		$this->load->model('product/compare');
		$products = $this->model_product_compare->getCompareProducts();
	...
	public function add() {
		...
		if (count($this->session->data['compare']) >= 4) {
			...
			$json['warning'] = sprintf($this->language->get('text_maximum_limit'),'4');
			...
			if ($product_info) {
				if (!in_array($this->request->post['product_id'], $this->session->data['compare'])) {
					if($this->customer->isLogged()){
						$res = $this->model_product_compare->getCompareProducts($this->request->post['product_id']);
						if($res){
							$json['warning'] = sprintf($this->language->get('text_error'), $this->url->link('product/product', 'product_id=' . $this->request->post['product_id']), $product_info['name'], $this->url->link('product/compare'));
						}else{
							$this->model_product_compare->addCompareProduct($this->request->post['product_id']);
	...
	public function getTotalCompareProduct(){
		$json['totalProducts'] = count($this->session->data['compare']);
