...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if ($category_info) {
			...
			if($data['pagination']!=''){
				$data['pagination_url'] = html_entity_decode($category_pagination_url, ENT_QUOTES, 'UTF-8');
			}
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/error/category_not_found.tpl')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/error/category_not_found.tpl', $data));
			} else {
	...
	public function listing() {
		...
		$data['order'] = $order;
		$data['limit'] = $limit;
		if (isset($this->request->get['display_view'])) {
			$data['display_view'] = $this->request->get['display_view'];
		}else{
			$data['display_view'] = 'grid-view';
		}
		$result = '';
		if(count($data['products'])<=0){
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/listing.tpl')) {
				$result = $this->load->view($this->config->get('config_template') . '/template/product/listing.tpl', $data);
			}
		}
		die($result);
	}
	...
	public function viewall(){
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('text_category_all'),
			'href' => $this->url->link('product/category/viewall')
		);

		$results = $this->model_catalog_category->getCategories();//fetch all
		foreach ($results as $result) {
			$filter_data = array(
				'filter_category_id'  => $result['category_id'],
				'filter_sub_category' => false
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/category-viewall.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/product/category-viewall.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/product/category.tpl', $data));
		}
	}
