...
var jQueryTab = {
	...
	handleEvent: function(obj){
		...
		if(!n){
			return ; /*javascript:void(0) found in url tab and tab is clicked*/
		}
		var self = obj;
	...
	addHeader: function(){
		...
		$(self.opts.tabClass).find("a").each(function(index, elem){													// iterate for each  tabs
			var $tab_content = $(self.opts.contentClass).eq($(this).parent().index());	
			//CUSTOM UPDATE
			var str = $(elem).attr('href');
			var n = str.indexOf("javascript");
			var href_url = '#'+$tab_content.attr('id');
			if(n){
				href_url = $(elem).attr('href');
			}
			...
			$('<a />').attr({																				// create a element with jquery
							...
							'href' : href_url											// set href attribute of header - works as named anchor
							})
