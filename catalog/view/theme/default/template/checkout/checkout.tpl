    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            if (json['redirect']) {
                ...
                if (json['error']['warning']) {
                    $('#collapse-shipping-method .panel-body').prepend('<div class="alert alert-warning">' + json['error']['warning'] + '<button type="button" class="close" data-dismiss="alert"></button></div>');
                }
            } else {
                $.ajax({
                    url: 'index.php?route=checkout/payment_method',
