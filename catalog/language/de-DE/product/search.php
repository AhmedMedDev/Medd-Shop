<?php


// Heading
$_['heading_title']		= 'Suche';
$_['heading_tag']		= 'Etikett - ';

// Text
$_['text_search']		= 'Produkte die den Suchkriterien entsprechen';
$_['text_keyword']		= 'Suchbegriff(e)';
$_['text_category']		= 'Alle Kategorien';
$_['text_sub_category']	= 'In Unterkategorien suchen';
$_['text_empty']        = '<h3>Es gibt kein Produkt, das den Suchkriterien entspricht.</h3> <p>Nicht gefunden, wonach Sie suchen? <a id="request_quote" onclick="getProductQuote(\'\', 2)">Klick hier</a> um Ihre Anfrage jetzt einzureichen.</a></p>';
$_['text_quantity']		= 'Menge';
$_['text_manufacturer']	= 'Hersteller';
$_['text_model']		= 'Artikelnr.';
$_['text_points']		= 'Bonuspunkte';
$_['text_price']		= 'Preis';
$_['text_tax']			= 'Preis ohne Steuer';
$_['text_reviews']		= '%s Beurteilungen';
$_['text_compare']		= 'Produktvergleich (%s)';
$_['text_sort']			= 'Sortieren nach';
$_['text_default']		= 'Vorgabe';
$_['text_name_asc']		= 'Name (A &gt; Z)';
$_['text_name_desc']	= 'Name (Z &gt; A)';
$_['text_price_asc']	= 'Preis (Niedrig &gt; Hoch)';
$_['text_price_desc']	= 'Preis (Hoch &gt; Niedrig)';
$_['text_rating_asc']	= 'Bewertung (Niedrig)';
$_['text_rating_desc']	= 'Bewertung (Hoch)';
$_['text_model_asc']	= 'Artikelnr. (A &gt; Z)';
$_['text_model_desc']	= 'Artikelnr. (Z &gt; A)';
$_['text_limit']		= 'Anzeige';
$_['text_load_more']    = 'Mehr laden';
$_['text_loading']      = 'Wird geladen...';
// Entry
$_['entry_search']		= 'Suche';
$_['entry_description']	= 'In Produktbeschreibungen suchen';