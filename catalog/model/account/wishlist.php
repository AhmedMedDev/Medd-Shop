...
class ModelAccountWishlist extends Model {
	...
	public function getWishlist($product_id=0) {
		if($product_id>0){
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_wishlist WHERE product_id = '" . (int)$product_id . "' AND customer_id = '" . (int)$this->customer->getId() . "'");
		}else{
