...
class ModelAccountReturn extends Model {
	...
	public function checkIfRequestAlreadyPlaced($orderId, $productCode)
	{
		$query = $this->db->query("SELECT r.return_id FROM `" . DB_PREFIX . "return` r WHERE order_id = '" . (int)$orderId .  "' AND customer_id = '" . $this->customer->getId() . "' AND return_status_id = '" . (int)$this->config->get('config_return_status_id') . "' AND model = '" . $this->db->escape($productCode) . "'");
