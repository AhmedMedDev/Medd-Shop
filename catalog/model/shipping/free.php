...
class ModelShippingFree extends Model {
	...
	function getFreeShippingQuote($address) {
		$this->load->language('shipping/free');
		...
		/**We're just showing message so we need not be worried about this check**/
		/**
		if ($this->cart->getSubTotal() < $this->config->get('free_total')) {
			$status = false;
		}
		**/
		...
		if ($status) {
			...
			$method_data = array(
				'code'       => 'free',
				'title'      => $this->language->get('text_title'),
				'min_order_val_text' => $this->currency->format($this->config->get('free_total')),
				'min_order_val' => $this->config->get('free_total')
			);
		}
