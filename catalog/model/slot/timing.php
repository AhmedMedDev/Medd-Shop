...
class ModelSlotTiming extends Model
{
    ...
    {
        $sql = "select show_slots,delivery_starts,time_format,date_format,time_zone,range_date_from from " . DB_PREFIX . "slot_setting";
        ...
        if ($query->num_rows>0) {
            ...
            $range_date_from = $query->row['range_date_from'];
        }
        ...
        $slotStartTime = date('Y-m-d H:i:s', strtotime($range_date_from));
        $added_lead_time = date('Y-m-d H:i:s', strtotime("+$lead hour"));
        $fromSlotTime = $slotStartTime;
        if ($slotStartTime < $added_lead_time) {
            $fromSlotTime = $added_lead_time;
        }

        $sql1 = "SELECT * from " . DB_PREFIX . "one_year_slot WHERE from_date_time>='".$fromSlotTime."' ORDER BY from_date_time asc LIMIT $limit";
