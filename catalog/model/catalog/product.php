...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array(), $applyPriceRangeFilter = true) {
		...
		//[custom code = price_range]

		if($applyPriceRangeFilter == true && isset($data['price_range']) && $data['price_range']!=''){
			$price_range  = explode("-", $data['price_range']);
			$price_1 = (int) $price_range[0];
			$price_2 = (int) $price_range[1];
			$sql .=" AND p.price BETWEEN '".$price_1."' AND '".$price_2."'";
		}
		...
		//[custom code = filter by multiple manufacturer ids]
		if (!empty($data['mid'])) {
			$manufacturers_id  = $data['mid'];
			$sql .= " AND p.manufacturer_id IN (" . $manufacturers_id . ")";
		}


		/* $sql .= " AND p.quantity > 0 GROUP BY p.product_id"; */ /* to show out of stock product having 0 quantity on listing pages*/
	...
	public function getTotalProducts($data = array()) {
		...
		if(isset($data['price_range']) && $data['price_range']!=''){
			$price_range  = explode("-", $data['price_range']);
			$price_1 = (int) $price_range[0];
			$price_2 = (int) $price_range[1];
			$sql .=" AND p.price BETWEEN '".$price_1."' AND '".$price_2."'";
