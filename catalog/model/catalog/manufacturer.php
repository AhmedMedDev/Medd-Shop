...
class ModelCatalogManufacturer extends Model {
	...
	public function getManufacturersByCategoryId($data = array()) {
		$product_data = array();
		if (!empty($data) && array_key_exists('filter_category_id', $data)) {
			
			$sql = " SELECT distinct p.manufacturer_id, m.name, m.image FROM ". DB_PREFIX ."product_to_category p2c  LEFT JOIN ". DB_PREFIX ."product p ON p.product_id = p2c.product_id LEFT JOIN ". DB_PREFIX ."manufacturer m ON p.manufacturer_id=m.manufacturer_id WHERE p2c.category_id = '".(int)$data['filter_category_id']."' AND p.manufacturer_id>0";
			$sort_data = array(
				'name',
				'sort_order'
			);
			...
			#echo "<pre>".print_r($product_data, true); die;
			return $product_data;
		}
	}
