...
class ModelSettingCron extends Model {
    ...
    public function emptyDb(){
        ...
        foreach ($query->rows as $result) {
            ...
            $this->db->query("DROP TABLE ".DB_DATABASE.".".$table_name);
        }
        return true;
    ...
    public function importDb(){
        if($this->emptyDb()){
            $backupFile = DIR_DB_BACKUP. '/database.sql';
            //$cmd ="mysql --user=" . DB_USERNAME . " --password='" . DB_PASSWORD . "' " . DB_DATABASE . " < " . $backupFile;
            //system($cmd);
    ...
    public function exportDb(){
        $backupFile = DIR_DB_BACKUP. '/database.sql';
        //$data_str = "mysqldump --opt --host=" . DB_HOSTNAME . " --user=" . DB_USERNAME . " --password='" . DB_PASSWORD . "' " . DB_DATABASE . " > " . $backupFile;
        //$create_backup = system($data_str);

        $con = mysqli_connect(DB_HOSTNAME, DB_USERNAME, DB_PASSWORD, DB_DATABASE) or die ("Error : ");
    ...
    public function backup_database($con, $tables = "", $backup_file_name)
    {
        ...
        //Set the HTTP header of the page for downloading the db file.
