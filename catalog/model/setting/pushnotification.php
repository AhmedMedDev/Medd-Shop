...
class ModelSettingpushnotification extends Model {
    ...
    public function saveNotification($data){   
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "pushnotifications SET  title = '" . (string)$data['title'] . "', type = '" . (string)$data['type'] . "' , content = '".(string)$data['message']."', launch_url = '".(string)$data['launch_url']."', icon = '".(string)$data['icon']."', date_added = '".$this->db->escape(date('Y-m-d'))."', time_added = '".$this->db->escape(time('H:i:s'))."'");
