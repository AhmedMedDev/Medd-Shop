<?php
class ModelAPITotalCoupon extends Model {
	...
	public function getCoupons($data = array()) {
		...
		$sql = "SELECT coupon_id, name, code, type,discount, date_start, date_end, status FROM " . DB_PREFIX . "coupon WHERE status=1 AND date_start <= now() AND date_end >= now()";
	...
	public function getTotal(&$total_data, &$total, &$taxes,&$cart_id='') {
		...
		$getCouponCode = $this->getCouponCode($cart_id);
		
		if (isset($getCouponCode['coupon_code'])) {
			$this->load->language('total/coupon');

			$coupon_info = $this->getCoupon($getCouponCode['coupon_code']);
			...
			if ($coupon_info) {
				...
				if ($discount_total > 0) {
					$total_data[] = array(
						'code'       => 'coupon',
						'title'      => sprintf($this->language->get('text_coupon'), $getCouponCode['coupon_code']),
	...
	public function applyCoupon($cart_id,$coupon_code){
		if($cart_id){
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "cart_coupon` WHERE user_id = '" . (int)$this->customer->getId() . "'");
			
			if($query->num_rows){
				$this->db->query("UPDATE " . DB_PREFIX . "cart_coupon SET coupon_code = '" . $coupon_code . "', cart_id = '".$cart_id."' WHERE user_id = '" . (int)$this->customer->getId() . "'");
			}else{
				$this->db->query("INSERT INTO `" . DB_PREFIX . "cart_coupon` SET user_id = '".(int)$this->customer->getId()."', cart_id = '".$cart_id."', coupon_code = '".$coupon_code."'");
			}
		}
	}
	...
	public function deleteCoupon($cart_id){
		$this->db->query("DELETE FROM `" . DB_PREFIX . "cart_coupon` WHERE user_id = '".(int)$this->customer->getId()."' AND cart_id = '".$cart_id."'");
