<?php
class ModelAPIAccountMessage extends Model {
	public function sendMessage($data) {
	
		$this->db->query("INSERT INTO " . DB_PREFIX . "msp_message SET customer_id = '" . (int)$data['customer_id'] . "', 
		message = '" . $data['message'] . "', sender = 'customer', date_added=NOW()");
	}
	...
	public function countTotalMessage($data){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "msp_message WHERE customer_id = '" . (int)$data['customer_id'] . "' AND hide_customer='0' ORDER BY date_added DESC");
		//echo 'test';
