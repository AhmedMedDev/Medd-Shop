<?php
class ModelModuleDeliveryTimeOrder extends Model {
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info) {
				...
				foreach ($order_total_query->rows as $total) {
					$data['totals'][] = array(
						...
						'text'  => $this->currency->format($total['value'], $order_info['currency_code'], $order_info['currency_value']),
                                ...
                                $order_total_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "delivery_time` WHERE order_id = '" . (int)$order_id . "'");

				foreach ($order_total_query->rows as $total) {

                                    $data['delivery_time'] = $total['delivery_time'];
				}

                                $data['time_zone'] = $_SESSION['time_zone'];

                                $this->load->model('module/deliveryTime/order');

                                $data['delivery_time'] = $this->model_module_deliveryTime_order->getDeliveryTime($order_id);
			...
			$extension = $this->model_slot_timing->getInstalled("module");
                        ...
                        if (!in_array("timeslot", $extension)) {
                            ...
                            $html = $this->load->view('default/template/mail/order.tpl', $data);
                                ...
                                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/timeslot/order.tpl')) {


                                    $html = $this->load->view($this->config->get('config_template') . '/template/module/timeslot/order.tpl', $data);
				...
				if (!$this->model_payment_amazon_checkout->isAmazonOrder($order_info['order_id'])) {
    				...
    				$mailHelper->sendEmail($order_info['email'], $subject, $html, $text);
				...
				if ($this->config->get('config_order_mail')) {
                    ...
                    $mailHelper = new MailHelper($this->config);
    				$mailHelper->setSenderName($order_info['store_name']);
    				$mailHelper->sendEmail($this->config->get('config_email'), $subject, $html, $text);
			...
			if ($order_info['order_status_id'] && $order_status_id && $notify) {
                ...
                $mailHelper = new MailHelper($this->config);
                $mailHelper->setSenderName($order_info['store_name']);
                //$mailHelper->sendEmail($order_info['email'], $subject, '', $message);
                $mailHelper->sendEmail($order_info['email'], $subject, $message, '');
        ...
        public function addDeliveryTime($order_id,$deliveryTime){
            ...
            $times = explode(" - ",$_SESSION['delivery_time']);
			...
			$deliveryTimeArray = explode(" - ",$this->db->escape($deliveryTime));
			$delivery_slot_time = date('y-m-d H:i:s',strtotime($deliveryTimeArray[0])).' - '.date('y-m-d H:i:s',strtotime($deliveryTimeArray[1]));

            $this->db->query("INSERT INTO " . DB_PREFIX . "delivery_time " . "SET order_id = '" . (int)$order_id . "', delivery_time = '" . $this->db->escape($deliveryTime) . "', delivery_slot_time = '" . $delivery_slot_time . "', times='".$new_time."'");


          /*   $this->db->query("INSERT INTO " . DB_PREFIX . "delivery_time " . "SET order_id = '" . (int)$order_id . "', delivery_time = '" . $this->db->escape($deliveryTime) . "', times='".$new_time."'"); */
