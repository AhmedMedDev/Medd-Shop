<?php
    class ControllerModuleTimeslot extends Controller {
...

        {
            ...
            // Default function
            $this->load->language('module/timeslot'); // Loading the language file of timeslot
            ...
            if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate())
            {
                ...
                $to_time = $this->request->post['to_time'];
                $from_time = $this->request->post['from_time'];



                $this->model_slot_timeslot->saveTimeSlot($from_time, $to_time);
                ...
                $this->model_slot_timeslot->SaveIntervelSettings($start_time,$end_time,$numberofslots,$order_per_slot,$time_format ,$date_range_from , $date_range_to ,$date_format , $time_zone, $lead_time);
                /*------------------------------------------------*/
                ...
                $this->model_slot_timeslot->setSlot($start_time, $end_time, $date_range_from , $date_range_to, $order_per_slot);
                ...
                $this->response->redirect($this->url->link('module/timeslot&success=successfully saved', 'token=' . $this->session->data['token'], 'SSL'));
            ...
            /* $deliveryTime = '2018-01-20 7:00:00 AM  - 2018-01-20  10:00:00 AM';
                $deliveryTime = $this->db->escape($deliveryTime);
                $deliveryTimeArray = explode(" - ",$deliveryTime);
                print_r($deliveryTimeArray);
                echo '<br />';
            echo date('y-m-d H:i:s',strtotime($deliveryTimeArray[0])).' - '.date('y-m-d H:i:s',strtotime($deliveryTimeArray[1])); */

            /* $getOrderSlotId = $this->model_slot_timeslot->getOrderSlotIdCommaSeperated();
            echo $getOrderSlotId; */
			...
			$timeSlots = $this->model_slot_timeslot->getTimeSlot();
			...
			$data['text_ddt'] = $this->language->get('text_ddt');
			$data['text_det'] = $this->language->get('text_det');
			$data['text_ddt_settings'] = $this->language->get('text_ddt_settings');
			$data['text_date_range'] = $this->language->get('text_date_range');
			$data['text_from'] = $this->language->get('text_from');
			$data['text_to'] = $this->language->get('text_to');
			$data['start_date'] = $this->language->get('start_date');
			$data['end_date'] = $this->language->get('end_date');
			$data['delivery_slot'] = $this->language->get('delivery_slot');
			$data['text_slot_interval_duration'] = $this->language->get('text_slot_interval_duration');
			$data['text_lead_time'] = $this->language->get('text_lead_time');
			$data['text_number_slot_display'] = $this->language->get('text_number_slot_display');
			$data['text_max_order_per_slot'] = $this->language->get('text_max_order_per_slot');
			$data['text_time_format'] = $this->language->get('text_time_format');
			$data['text_date_format'] = $this->language->get('text_date_format');
			$data['text_time_zone'] = $this->language->get('text_time_zone');
			$data['text_shop_opening_days'] = $this->language->get('text_shop_opening_days');
			...
			$data['text_value_to_date'] = $this->language->get('text_value_to_date');
			$data['text_add_exclude_btn'] = $this->language->get('text_add_exclude_btn');
			$data['text_all_slots'] = $this->language->get('text_all_slots');
			$data['text_change_max_order'] = $this->language->get('text_change_max_order');
            ...
            'href'      => $this->url->link('module/timeslot', 'token=' . $this->session->data['token'], 'SSL'),
            'separator' => ' :: '
			);

			/* End Breadcrumb Block*/

			$data['action'] = $this->url->link('module/timeslot', 'token=' . $this->session->data['token'], 'SSL'); // URL to be directed when the save button is pressed
			...
			$this->response->setOutput($this->load->view('module/timeslot.tpl', $data));
        }
