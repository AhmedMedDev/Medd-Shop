			...
			if (!$json) {
				...
				switch ($this->request->post['to']) {
					case 'newsletter':
						...
						$newsletterResults=$this->model_common_newsletter->getNewsLetter();
						if(!empty($newsletterResults)) {
							
							foreach($newsletterResults as $res)
							{								 
								$emails[] = $res['email'];								
							}
						}
				...
				if ($emails) {
					...
					foreach ($emails as $email) {
						if (preg_match('/^[^\@]+@.*.[a-z]{2,15}$/i', $email)) {
							...
							$mailHelper->sendEmail($email, $this->request->post['subject'], $message);
