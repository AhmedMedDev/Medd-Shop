...
class ModelLocalisationLanguage extends Model {
	...
	public function getLanguages($data = array(), $listing = false) {
        
        if($listing) {
                $base_sql = "SELECT * FROM " . DB_PREFIX . "language";
            }
            else {
                $base_sql = "SELECT * FROM " . DB_PREFIX . "language where status = '1'";
        }
		...
		if ($data) {
			...
			if (!$language_data) {
				...
				$query = $this->db->query($base_sql." ORDER BY sort_order, name");
