...
class ModelSlotTimeslot extends Model {
	...
	public function setSlot($start_time,$end_time,$date_range_from , $date_range_to , $order_per_slot)
	{
		$this->setTables();
		$makeSlot = $this->makeSlot($start_time, $end_time ,  $date_range_from , $date_range_to, $from_time, $to_time);
		if($makeSlot!=""){
			$this->SaveSlot($makeSlot,$start_time , $end_time, $order_per_slot);
		}
	}

	public function makeSlot($start , $end , $date_range_from , $date_range_to){
		// choose time zone
		...
		for($i=0;$i <= $total; $i++)
		{
			...
			$datas[] = $this->makeOneDaySlots($start,$end , $exc_day[2] , $exc_day[1],$date_range_to , $date_range_from,$exc_day[0]);
		}
	...
	public function makeOneDaySlots($start, $end, $day, $month ,$date_range_to,$date_range_from,$year)
	{
		$data = array();
		...
		$slotResults = $this->getTimeSlot();
		//print_r($slotResults);
		$from_time = $slotResults['from_time'];
		$to_time = $slotResults['to_time'];
		$get_result = count($from_time)-1;

		for($i=0; $i<=$get_result; $i++):
			/* $inter +=$interval;
			$new_val =+ $inter + $interval; */
			$save =  date("$year-$month-$day")." ".$from_time[$i]." "
			. "- ".date("$year-$month-$day")." ".$to_time[$i];

			$ToDate = date("$year-$month-$day")." ".$from_time[$i];
			$fromDate = date("$year-$month-$day")." ".$to_time[$i];
	...
	public function saveTimeSlot($from_time, $to_time){
		$query = $this->db->query("TRUNCATE TABLE " . DB_PREFIX . "time_slot");

		$get_result = count($from_time)-1;
		for($i=0; $i<=$get_result; $i++){
			$query = $this->db->query("INSERT  " . DB_PREFIX . "time_slot (from_time , to_time) values('".$from_time[$i]."' , '".$to_time[$i]."' )");
		}
	}


	public function getTimeSlot(){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "time_slot");
		$results = $query->rows;
		...
		return array('from_time'=>$from_time,'to_time'=>$to_time);
	}

	public function SaveSlot($arr , $start_time , $end_time , $order_per_slot){
		// First Truncate the table
		//$query = $this->db->query("TRUNCATE TABLE " . DB_PREFIX . "one_year_slot");


		$getOrderSlotId = $this->getOrderSlotIdCommaSeperated();

		$query = $this->db->query("DELETE from " . DB_PREFIX . "one_year_slot WHERE one_slot_id NOT IN ($getOrderSlotId)");
		foreach ($arr as $val):
			$i=0;
			foreach ($val['slots'] as $vals):
				...
				$getVal = $this->db->query("SELECT * FROM " . DB_PREFIX . "one_year_slot p WHERE p.slot_timing = '".$slot_timing."'");

				if($getVal->num_rows === 0){
					$query = $this->db->query("INSERT  " . DB_PREFIX . "one_year_slot (slot_timing , to_date_time , from_date_time , date , max_no) values('".$slot_timing."' , '".$from."' , '".$to."' , '".$dates."' , '".$order_per_slot."' )");
				}
				$i++;
			endforeach;
		endforeach;
	...
	public function SaveIntervelSettings($start_time,$end_time,$numberofslots,$order_per_slot,$time_format , $date_range_from , $date_range_to , $date_format , $time_zone, $delivery_starts_lead_time){
		$Slots = $this->db->query("SELECT * FROM " . DB_PREFIX . "slot_setting");
		if($Slots->num_rows>0)
		{
			$this->db->query("UPDATE " . DB_PREFIX . "slot_setting SET end_time='".$end_time."' , start_time='".$start_time."' , show_slots='".$numberofslots."' , order_per_slot='".$order_per_slot."' , time_format='".$time_format."', delivery_starts='".$delivery_starts_lead_time."', "
				. "range_date_from ='".$date_range_from."' , range_date_to='".$date_range_to."' , time_zone='".$time_zone."' , date_format='".$date_format."' WHERE slot_id=1");

		}else{

			$this->db->query("insert into " . DB_PREFIX . "slot_setting(`end_time`,`start_time`,`show_slots`,"
				. "`order_per_slot` ,`time_format`, `delivery_starts` ,`date_range_from` , `date_range_to`, `date_format` ,`time_zone` ) "
			. "values ( '".$end_time."','".$start_time."','".$SaveIntervelSettings."','".$order_per_slot."' , '".$time_format."', '".$delivery_starts_lead_time."', '".$date_range_from."' , '".$date_range_to."' , '".$date_format."' , '".$time_zone."' )");
		}
	...
	public function GeneratedSlots(){
			...
			(SELECT COUNT(*) FROM " . DB_PREFIX . "delivery_time v WHERE delivery_slot_time LIKE p.slot_timing ) as count
			FROM    " . DB_PREFIX . "one_year_slot p");
		   return $query;
	}

	public function getOrderSlotIdCommaSeperated(){

		$query = $this->db->query("SELECT  p.one_slot_id FROM " . DB_PREFIX . "one_year_slot p INNER JOIN " . DB_PREFIX . "delivery_time v ON v.delivery_slot_time = p.slot_timing GROUP BY p.one_slot_id");
		foreach($query->rows as $rows){
			$ids[] = $rows['one_slot_id'];
		}
		$ids = implode (",", $ids);
		return $ids;
	}
	...
	public function setTables(){
		...
		$this->db->query("insert into " . DB_PREFIX . "slot_setting(`slot_id`,`end_time`,`intervals`,`delivery_starts`,`start_time`,`show_slots`,`order_per_slot`,`time_format` , `range_date_from` , `range_date_to` , `date_format` , `time_zone`) values ( NULL,'22:00:00','2','2','01:00:00','30','1','12 ( AM/PM and O clock )','".date('Y-m-d')."','".date('Y-m-d', strtotime('+12 months'))."','yy-mm-dd','Asia/karachi')");
