...
class ModelCatalogOption extends Model {
	public function addOption($data) {
		...
		foreach ($data['option_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "option_description SET option_id = '" . (int)$option_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape(trim($value['name'])) . "'");
		}
		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				...
				foreach ($option_value['option_value_description'] as $language_id => $option_value_description) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value_description SET option_value_id = '" . (int)$option_value_id . "', language_id = '" . (int)$language_id . "', option_id = '" . (int)$option_id . "', name = '" . $this->db->escape(trim($option_value_description['name'])) . "'");
				}
			}
		}
	...
	public function getOptions($data = array()) {
		...
		if (!empty($data['filter_name'])) {
			$sql .= " AND od.name LIKE '" . $this->db->escape(trim($data['filter_name'])) . "%'";
		}
