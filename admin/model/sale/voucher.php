	...
	public function sendVoucher($voucher_id) {
		...
		if ($voucher_info) {
			...
			if ($order_info) {
				...
				$html = $this->load->view('mail/voucher.tpl', $data);
				$mailHelper->sendEmail($voucher_info['to_email'], sprintf($this->language->get('text_subject'), $voucher_info['from_name']), $html);
				...
				$mail->setSubject(sprintf($this->language->get('text_subject'), html_entity_decode($voucher_info['from_name'], ENT_QUOTES, 'UTF-8')));
