...
class ModelSaleReturn extends Model {
	...
	public function addReturnHistory($return_id, $data) {
        ...
        $rData = $this->getReturn($return_id);
        if($data['return_status_id'] == 3) {
        
            $this->db->query("UPDATE `" . DB_PREFIX . "product` SET quantity = (quantity + " . (int)$rData['quantity'] . ") WHERE product_id = '" . (int)$rData['product_id'] . "' AND subtract = '1'");

            $option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$rData['order_id'] . "' AND order_product_id = '" . (int)$rData['product_id'] . "'");
		...
		if ($data['notify']) {
			...
			if ($return_query->num_rows) {
				...
				//$mailHelper->sendEmail($return_query->row['email'], $subject, '', $message);
				$mailHelper->sendEmail($return_query->row['email'], $subject, $message, '');
