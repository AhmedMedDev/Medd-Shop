...
class ModelMarketingAffiliate extends Model {
	...
	public function approve($affiliate_id) {
		...
		if ($affiliate_info) {
			...
			//$mailHelper->sendEmail($affiliate_info['email'], sprintf($this->language->get('text_approve_subject'), $this->config->get('config_name')), '', $message);
			$mailHelper->sendEmail($affiliate_info['email'], sprintf($this->language->get('text_approve_subject'), $this->config->get('config_name')), $message, '');
	...
	public function addTransaction($affiliate_id, $description = '', $amount = '', $order_id = 0) {
		...
		if ($affiliate_info) {
			...
			//$mailHelper->sendEmail($affiliate_info['email'], sprintf($this->language->get('text_transaction_subject'), $this->config->get('config_name')), '', $message);
			$mailHelper->sendEmail($affiliate_info['email'], sprintf($this->language->get('text_transaction_subject'), $this->config->get('config_name')), $message, '');
